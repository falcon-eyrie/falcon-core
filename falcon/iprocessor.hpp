  ...
  protected: // callable by derived processors, but not others
    ...
    create_output_port(std::string name,
        ...
        output_ports_[name] = std::move(
            std::unique_ptr<IPortOut>((IPortOut *)new PortOut<DATATYPE>(
    ...
    create_output_port(const typename DATATYPE::Parameters &parameters,
                       const PortOutPolicy &policy) {
        return create_output_port<DATATYPE>(DATATYPE::dataname(), parameters,
                                            policy);
    }
     ...
     */
    IPortIn *input_port(std::string port) {
        return input_ports_.at(port).get();
    }
    ...
    */
    ...
    ProducerState<T> *create_producer_state(
        ...
        if (cooperative) {
            return ((ProducerState<T> *)create_writable_shared_state<T>(
                state, default_value, Permission::WRITE, external,
                description));
        } else {
            return ((ProducerState<T> *)create_writable_shared_state<T>(
                state, default_value, Permission::NONE, external, description));
    ...
    template <typename T>
    ReadableState<T> *
    create_readable_shared_state(std::string state, T default_value,
