...
void SlotOut<DATATYPE>::CreateRingBuffer(int buffer_size, WaitStrategy wait_strategy) {
    ...
    try {
        ringbuffer_.reset(new RingBuffer<typename DATATYPE::Data>(
            prototype(), buffer_size_, ClaimStrategy::kSingleThreadedStrategy, wait_strategy));
...
bool SlotIn<DATATYPE>::RetrieveData(typename DATATYPE::Data*& data, int64_t time_out) {
    ...
    try {
        if (time_out < 0) {
            ...
            int64_t available_sequence = upstream_->WaitFor(requested_sequence, time_out);
...
void PortIn<DATATYPE>::Connect(int slot, ISlotOut* upstream) {
    ...
    if (slot < 0 || slot >= number_of_slots()) {
        throw std::out_of_range(
            "Error connecting to slot " + std::to_string(slot) +
            " (number of available slots = " + std::to_string(number_of_slots()) + ")");
    }
...
void PortIn<DATATYPE>::NewSlot(int n) {
    ...
    for (int k = 0; k < n; k++) {
        ...
        auto slot = std::make_unique<SlotIn<DATATYPE>>(this, address, capabilities_,
